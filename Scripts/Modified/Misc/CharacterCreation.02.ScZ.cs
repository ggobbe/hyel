/*12-02-08 Modifications par Plume
*/
using System;
using Server;
using Server.Items;
...
namespace Server.Misc
{
	public class CharacterCreation
	{
		...
		private static void AddBackpack( Mobile m )
		{
            ...
            /** 
             * Scriptiz : Changement des objets de départ
             */
			PackItem( new RedBook( "a book", m.Name, 20, true ) );
            PackItem(new Apple(2));
			PackItem( new Gold( 1000 ) ); // Starting gold can be customized here
			PackItem( new Dagger() );
			//PackItem( new Candle() );
            EquipItem(new Robe(Utility.RandomDyedHue()));
            
            // ajout jupe ou short selon le sexe
            if(m.Female)
                EquipItem(new Skirt(Utility.RandomDyedHue()));
            else
                EquipItem(new ShortPants(Utility.RandomDyedHue()));
		...
		private static void FillBankAOS( Mobile m )
		{
            ...
            PlaceItemIn(cont, 48, 55, new RunicSewingKit(CraftResource.AmphibianLeather, 1000));
            PlaceItemIn(cont, 48, 55, new RunicSewingKit(CraftResource.DaemonLeather, 1000));
            PlaceItemIn(cont, 48, 55, new RunicSewingKit(CraftResource.DarkLeather, 1000));
            PlaceItemIn(cont, 48, 55, new RunicSewingKit(CraftResource.ReptileLeather, 1000));
            ...
            PlaceItemIn(cont, 92, 60, new ReptileLeather(5000));
            PlaceItemIn(cont, 92, 60, new DarkLeather(5000));
            PlaceItemIn(cont, 92, 60, new DaemonLeather(5000));
            PlaceItemIn( cont, 92, 60, new AmphibianLeather(5000));
		...
		private static void EventSink_CharacterCreated( CharacterCreatedEventArgs args )
		{
			...
			if( Core.Expansion >= args.Race.RequiredExpansion ) {
				...
				if( args.Race == Race.Elf ) {
					newChar.BodyValue = args.Female ? 0x0B8 : 0x0B7;
				}
				else {
					newChar.BodyValue = args.Female ? 0x191 : 0x190;
				}
			}
			else {
				newChar.Race = Race.DefaultRace;
			...
			newChar.Hue = ( args.Hue & 0x3FFF ) | 0x8000;
			...
			newChar.MoveToWorld( new Point3D( 3288, 3891, 0 ), Map.Felucca );
