...
{
    public class DPNobleSacrificeSpell : DarkPaladinSpell
	{
		private static SpellInfo m_Info = new SpellInfo(
                "Don Malefique", "Maleficia Resta",
				-1,
				9002
			);
        ...
        public DPNobleSacrificeSpell(Mobile caster, Item scroll)
		...
		public override void OnCast()
		{
			if ( CheckSequence() )
			{
				...
				double resChance = 0.1 + (0.9 * ((double)Caster.Karma*-1 / 10000));
				...
				for ( int i = 0; i < targets.Count; ++i )
				{
					...
					else
					{
						...
						if ( m.Hits < m.HitsMax )
						{
							int toHeal = DarkComputePowerValue( 10 ) + Utility.RandomMinMax( 0, 2 );
