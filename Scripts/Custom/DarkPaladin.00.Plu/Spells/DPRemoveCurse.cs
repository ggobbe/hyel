...
{
    public class DPRemoveCurseSpell : DarkPaladinSpell
	{
		private static SpellInfo m_Info = new SpellInfo(
                "Lever la Malediction", "Maleficia Exterma",
				-1,
				9002
			);
        ...
        public DPRemoveCurseSpell(Mobile caster, Item scroll)
		...
		public void Target( Mobile m )
		{
			if ( CheckBSequence( m ) )
			{
                ...
                if (Caster.Karma > 5000)
					...
					chance = (int) Math.Sqrt( 20000 + (Caster.Karma*-1) ) - 122;
				else if ( Caster.Karma < -5625 )
					chance = (int) Math.Sqrt( Caster.Karma*-1 ) + 25;
				else
					chance = 100;
		...
		{
			private DPRemoveCurseSpell m_Owner;

			public InternalTarget( DPRemoveCurseSpell owner ) : base( 12, false, TargetFlags.Beneficial )
			{
				m_Owner = owner;
			}
