    ...
    public class VolcanicEruptionSpell : DruidSpell
   {
      ...
      public void Target( IPoint3D p )
      {
         ...
         else if ( SpellHelper.CheckTown( p, Caster ) && CheckSequence() )
         {
            ...
            IPooledEnumerable eable = Caster.Map.GetMobilesInRange( new Point3D( p ), 1 + (int)(Caster.Skills[SkillName.AnimalLore].Value / 10.0) );

            foreach ( Mobile m in eable )
            {
               if ( SpellHelper.ValidIndirectTarget( Caster, m ) && Caster.CanBeHarmful( m, false )  && (m != Caster))
                  targets.Add( m );
            }
