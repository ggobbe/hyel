...
{
    public class MurCactusSpell : DruidSpell
	{
		private static SpellInfo m_Info = new SpellInfo(
				"Mur de cactus", "Cak an Grav",
				230,
				9052,
      ...
      public override int RequiredMana{ get{ return 20; } }
		...
		public MurCactusSpell( Mobile caster, Item scroll ) : base( caster, scroll, m_Info )
		{
		}
		...
		public void Target( IPoint3D p )
		{
			...
			else if ( SpellHelper.CheckTown( p, Caster ) && CheckSequence() )
			{
				...
				for ( int i = -2; i <= 2; ++i )
				{
					...
					new InternalItem( 3374, loc, Caster, Caster.Map, duration, i );
		...
		private class InternalItem : Item
		{
			...
			public void ApplyPoisonTo( Mobile m )
			{
				...
				if ( Core.AOS )
				{
					double total = m_Caster.Skills[SkillName.Magery].Value + (((m_Caster.Skills[SkillName.Poisoning].Value)+(m_Caster.Skills[SkillName.AnimalLore].Value)) / 2);
		...
		{
			private MurCactusSpell m_Owner;

			public InternalTarget( MurCactusSpell owner ) : base( 12, true, TargetFlags.None )
			{
				m_Owner = owner;
			}
