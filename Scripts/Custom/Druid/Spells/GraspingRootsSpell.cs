    ...
    public class GraspingRootsSpell : DruidSpell
   {
      ...
      public void Target( Mobile m )
      {
         ...
         else if ( CheckHSequence( m ) )
         {
            ...
            duration = 9.0 + (Caster.Skills[SkillName.AnimalLore].Value * 0.4);

            // Resist if Str + Dex / 2 is greater than CastSkill eg. AnimalLore seems to work??
            if ( ( Caster.Skills[CastSkill].Value ) < ( ( m.Str + m.Dex ) * 0.1 ) )
               duration *= 2.5;
      ...
      private class InternalItem : Item
      {
         ...
         public InternalItem( Point3D loc, Map map, Mobile caster ) : base( 0xC5F )
         {
            ...
            m_Timer = new InternalTimer( this, TimeSpan.FromSeconds( 40.0 ) );
            m_Timer.Start();

            m_End = DateTime.Now + TimeSpan.FromSeconds( 40.0 );
         }
