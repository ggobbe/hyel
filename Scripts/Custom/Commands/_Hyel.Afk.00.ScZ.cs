...
namespace Server.Commands
{
    ...
    public class AFK : Timer
    {
        ...
        protected override void OnTick()
        {
            ...
            try
            {
                NetState fromState = who.NetState;
                if (ts.Minutes >= 5)
                {
                    fromState.Dispose();
                    Stop();
            ...
            who.Emote("*{0} (depuis {1}:{2}:{3}:{4})*", what, ts.Days, ts.Hours, ts.Minutes, ts.Seconds);
