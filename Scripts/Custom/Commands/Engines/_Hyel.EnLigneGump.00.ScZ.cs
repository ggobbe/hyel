/**
 * Scriptiz@19/11/07 : Script pour le .enligne du joueur
 * 
 * .enligne : affiche les joueurs connecté pour un joueur
 * .enligne : pareil que le .who pour les GM
 **/
using System;
...
namespace Server.Gumps
{
    public class EnLigneGump : Gump
    {
        ...
        {
            CommandSystem.Register("EnLigne", AccessLevel.Player, new CommandEventHandler(WhoList_OnCommand));
        }

        [Usage("EnLigne")]
        [Description("Affiche la liste des personnes en ligne")]
        ...
        public static readonly int PrevLabelOffsetY = 0;

        public static readonly int NextLabelOffsetX = -29;
        public static readonly int NextLabelOffsetY = 0;

        public static readonly int EntryWidth = 180;
        public static readonly int EntryCount = 15;

        public static readonly int TotalWidth = OffsetSize + EntryWidth + OffsetSize + SetWidth + OffsetSize;
        public static readonly int TotalHeight = OffsetSize + ((EntryHeight + OffsetSize) * (EntryCount + 1));

        public static readonly int BackWidth = BorderSize + TotalWidth + BorderSize;
        public static readonly int BackHeight = BorderSize + TotalHeight + BorderSize;
        ...
        public EnLigneGump(Mobile owner)
            : this(owner, BuildList(owner), 0)
        {
        }

        public EnLigneGump(Mobile owner, ArrayList list, int page)
            : base(GumpOffsetX, GumpOffsetY)
        {
            owner.CloseGump(typeof(EnLigneGump));
        ...
        public static ArrayList BuildList(Mobile owner)
        {
            ...
            for (int i = 0; i < states.Count; ++i)
            {
                ...
                if (m != null && (m == owner || owner.AccessLevel >= m.AccessLevel))
                    list.Add(m);
            }
        ...
        private static int GetHueFor(Mobile m)
        {
            ...
            {
                case AccessLevel.Administrator: return 0x58; //return 0x516;
                case AccessLevel.Seer: return 0x58; //return 0x144;
                case AccessLevel.GameMaster: return 0x58; //return 0x21;
                case AccessLevel.Counselor: return 0x58; //return 0x2;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                ...
                case 1: // Previous
                    {
                        if (m_Page > 0)
                            from.SendGump(new EnLigneGump(from, m_Mobiles, m_Page - 1));
                ...
                default:
                    {
                        ...
                        if (index >= 0 && index < m_Mobiles.Count)
                        {
                            ...
                            if (m.Deleted)
                            {
                                ...
                                from.SendGump(new EnLigneGump(from, m_Mobiles, m_Page));
                            }
